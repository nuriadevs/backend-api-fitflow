...
export const exerciseSearchToolDefinition = {
 ...
 description:
  'Searches for exercises and information about them, including title, type, bodypart, equipment,level, and desc. Use this to answer questions about exercises.',
};
...
export const exerciseSearch: ToolFn<Args, string> = async ({ toolArgs }) => {
 ...
 try {
  ...
  const exercises = limitedResults
   .map((result) => ({
